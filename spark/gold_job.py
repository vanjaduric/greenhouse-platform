"""
Gold Layer - Aggregated Insightful Metrics
Reads Silver Delta from MinIO, calculates 30-minute aggregates,
writes Gold Delta to MinIO AND summary rows to Postgres.
...
# Crop optimal ranges for climate score calculation
OPTIMAL_RANGES = {
    "tomato":   {"temperature": (20,26), "humidity": (65,75), "co2": (800,1100), "light": (200,600), "airflow": (0.2,0.8)},
    "cucumber": {"temperature": (22,28), "humidity": (70,80), "co2": (700,1000), "light": (150,500), "airflow": (0.2,0.7)}
}
...
# ---------------------------------------------------------------------------
# Step 1 — 30-minute aggregates per facility per sensor type
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 2 — Pivot so each sensor type becomes a column
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 3 — Join VPD averages for the same windows
# ---------------------------------------------------------------------------
...
gold_df = pivoted_df.join(
    vpd_windowed,
    ["facility_id", "window_start"],
    "left"
)
...
def within_range_score(value_col, low, high):
    ...
    return when(value_col.isNull(), None) \
        ...
        .when(value_col < low,
              spark_round(
                  (100 - (lit(low) - value_col) / range_size * 100).cast(FloatType()),
                  2
              )) \
        .otherwise(
            spark_round(
                (100 - (value_col - lit(high)) / range_size * 100).cast(FloatType()),
                2
            )
        )

# We use tomato ranges as default — in production this would be per crop_type
gold_df = gold_df \
    .withColumn("score_temperature",
        within_range_score(col("avg_temperature"), 20, 26)) \
    .withColumn("score_humidity",
        within_range_score(col("avg_humidity"), 65, 75)) \
    .withColumn("score_co2",
        within_range_score(col("avg_co2"), 800, 1100)) \
    .withColumn("score_light",
        within_range_score(col("avg_light"), 200, 600)) \
...
# ---------------------------------------------------------------------------
# Step 5 — Overall anomaly rate across all sensors
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Step 7 — Write Gold Delta to MinIO
# ---------------------------------------------------------------------------
...
def write_row_to_postgres(conn, row):
    with conn.cursor() as cur:
        ...
        """, (
            ...
            row.avg_vpd,
            row.climate_score,
            row.variance_temperature,
            row.overall_anomaly_rate,
            row.gold_processed_at
        ))
    conn.commit()

# Collect Gold rows and write to Postgres
# Gold is small (one row per facility per 30 min) so collect() is safe
