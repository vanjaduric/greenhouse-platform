...
An IoT data streaming platform simulating smart greenhouse monitoring, built around the architecture of commercial controlled-environment agriculture systems. Sensor data is generated by a mock API that simulates two facilities with realistic values, daily sinusoidal variation, and ~2% anomaly injection. The ingestion service polls the API every 10 seconds, routing readings to Kafka topics that feed two parallel paths: a real-time path writing live sensor values to Grafana dashboards, and a 30-minute batch pipeline processing raw data through Bronze, Silver, and Gold Spark jobs into aggregated climate metrics.

![Dashboard](assets/grafana_showcase.gif)
